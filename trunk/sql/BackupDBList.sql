...
GO
CREATE PROCEDURE BackupDBList 	@DBList 	nvarchar(1000), 
...
--	END
	...
	WHILE (1=1) BEGIN
		...
		DECLARE @device_name sysname,  @backup_path nvarchar(500) 
		SET @device_name = @dbname + 	'_' + convert(varchar(20),getdate(), 112) 
						+ '_' + replace(convert(varchar(20), getdate(), 108), ':', '')

		SET @backup_path = @device_name 

		SET @backup_path = @backup_path + '_' + lower(@BackupType) + '.bak'

		IF (right(@BackupDir, 1) <> '\') SET @BackupDir = @BackupDir + '\'
		SET @backup_path = @BackupDir + @backup_path
...
--		SET @logmsg = 'Failed to create dump device : ' + @vbackupdevice 
		...
		IF (upper(@BackupType) = 'FULL') BEGIN
			BACKUP DATABASE @dbname TO @device_name 
				WITH  INIT, NAME = @dbname, NOSKIP , STATS = 10, DESCRIPTION = @backup_path, NOFORMAT 
		END ELSE IF (upper(@BackupType) = 'LOG') BEGIN
			BACKUP LOG @dbname TO @device_name 	
				WITH  INIT, NAME = @dbname, NOSKIP , STATS = 10, DESCRIPTION = @backup_path, NOFORMAT 
		END ELSE IF (upper(@BackupType) = 'DIFF') BEGIN
			BACKUP DATABASE @dbname TO @device_name 
				WITH  INIT, DIFFERENTIAL, NAME = @dbname, NOSKIP , STATS = 10, DESCRIPTION = @backup_path, NOFORMAT 
